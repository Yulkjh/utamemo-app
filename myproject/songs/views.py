...
@login_required
def karaoke_separate(request, pk):
    """カラオケモード - Demucsでボーカル除去"""
    import os
    import tempfile
    import subprocess
    ...
    # 音声URLを取得
    audio_url = song.audio_url or (song.audio_file.url if song.audio_file else None)
    if not audio_url:
        return JsonResponse({'error': 'No audio available'}, status=400)
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(no_vocals_path):
                ...
                return FileResponse(
                    ...
                    as_attachment=False,
                    filename=f'{song.title}_karaoke.mp3'
                )
            else:
                raise FileNotFoundError('Separated audio not found')
                
        except subprocess.TimeoutExpired:
            return JsonResponse({'error': 'Processing timeout'}, status=504)
        except subprocess.CalledProcessError as e:
            logger.error(f'Demucs error: {e}')
            return JsonResponse({'error': 'Separation failed'}, status=500)
        finally:
            # クリーンアップ
        ...
        logger.error(f'Karaoke separation error: {e}')
