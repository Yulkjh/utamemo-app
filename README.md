...
│   ├── training_agent.py       # 学習エージェント (ダッシュボード連携・自動ループ)
│   ├── generate_history_data.py # Gemini API による学習データ自動生成
│   ├── test_model.py           # 学習済みモデルのテスト
│   ├── export_training_data.py # DB → 学習データ JSON 変換
│   ├── start_server.sh         # ワンコマンド起動 (serve.py + Cloudflare Tunnel)
│   ├── webui/                  # Gradio WebUI (学習管理・SSH連携)
│   │   ├── app.py              # Gradio アプリ
│   │   └── ssh_manager.py      # SSH 経由で学校 GPU 管理
│   ├── requirements_training.txt
│   └── data/
│       ├── sample_training_data.json
│       └── lyrics_training_data.json  # 本番学習データ (Gemini 生成含む)
...
スタッフ / スーパーユーザー権限でアクセスできる管理ページ:

| URL | ページ名 | 説明 |
|-----|---------|------|
| `/staff/llm-guide/` | スタッフポータル | 管理機能のハブページ |
| `/staff/training/` | LLM 学習ダッシュボード | 学習の開始・停止・ステータス監視 |
| `/staff/training-data/` | 学習データ確認 | 生成済み学習データの閲覧・検索 |
| `/api-status/` | API ステータス | LLM バックエンド接続状態 |

---

## トレーニングエージェント (training_agent.py)

ローカル PC で常駐し、Web ダッシュボード (`/staff/training/`) からリモートで学習を制御するエージェント。

### 仕組み
1. エージェントが 10 秒間隔でサーバー (`/api/training/update/`) にポーリング
2. ダッシュボードから「開始」→ エージェントが `train.py` を実行
3. **自動ループ**: 一度「開始」すると、学習完了後に自動で次のサイクルを開始
4. ダッシュボードから「停止」→ 自動ループを停止

### 起動方法
```bash
cd training
python training_agent.py \
  --api_key YOUR_TRAINING_API_KEY \
  --report_url https://utamemo.com/api/training/update/
```

### 環境変数
| 変数名 | 説明 |
|--------|------|
| `PYTHONIOENCODING` | `utf-8` に設定（Windows 環境で必須） |

---

## 学習データ自動生成 (generate_history_data.py)

Gemini API を使って日本史テーマから暗記用歌詞の学習データを自動生成するスクリプト。

### 使い方
```bash
cd training
python generate_history_data.py --gemini-key YOUR_GEMINI_KEY
```

### 動作
- 58 件のテーマ（日本史 33 件 + 世界史 7 件 + 理科 10 件 + 数学 3 件 + 英語 2 件 + 公民 2 件 + 地理 1 件）を定義
- Gemini API でテーマごとに歌詞を生成（pop / rock / hip-hop / EDM をローテーション）
- 既存の `data/lyrics_training_data.json` にマージ保存（現在 79 件）
- 各テーマに `keywords` を指定し、歌詞に必ずキーワードが含まれるよう指示

### 注意事項
- `gemini-3-flash-preview` を現在使用（~13 秒/件、安定）
- `gemini-2.5-flash-lite` も安定・高速（~5-7 秒/件）
- `gemini-2.5-flash` は使用可能だが応答が遅い場合あり
- `gemini-2.0-flash` は新規ユーザーに利用不可
...
## バージョン
2.3.0（2026 年 4 月 9 日）

## 更新履歴
- **v2.3.0** (2026/4/9)
  - Gemini API による学習データ自動生成ループ（ダッシュボードから開始 → データ生成 → LoRA 学習 → 自動繰り返し）
  - `generate_history_data.py --random-count N`: ランダムテーマ自動生成モード追加
  - トレーニングエージェントのバグ修正（stop コマンド優先度、クラッシュ復旧、中間 stop チェック、コマンド消失防止）
  - 学習データ: 82 件（9 教科）
  - Web 要件から torch/demucs を除去（デプロイ時間短縮）
- **v2.2.0** (2026/4/9)
  - トレーニングエージェント（ダッシュボード連携・自動ループ学習）
  - Gemini API による学習データ自動生成（9 教科 58 テーマ → 79 件）
  - 学習データ確認ページ（`/staff/training-data/`）
  - スタッフポータルハブページ（`/staff/llm-guide/`）UI 統一（オレンジテーマ）
  - Gradio WebUI（学習管理 UI）+ SSH 連携
  - ノート重要度スコアリングモジュール（ルールベース + LLM ハイブリッド）
- **v2.1.0** (2026/3/20)
