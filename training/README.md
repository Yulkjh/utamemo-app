...
学校のGPU PC (RTX 4080 × 2) と自宅PC (RTX 4060 Ti) を使って、Gemini APIの代わりに歌詞を生成するローカルLLMをセットアップする手順。

## 🖥️ GPU環境

| 場所 | GPU | VRAM | 用途 |
|------|-----|------|------|
| 🏫 学校 | RTX 4080 × 2 | 16GB × 2 = 32GB | 大規模モデルの学習・推論、マルチGPU |
| 🏠 自宅 | RTX 4060 Ti × 1 | 16GB | 学習・推論・開発 (Llama 3 8Bが動く) |
...
**🏠 自宅 RTX 4060 Ti (16GB VRAM):**

16GB VRAMがあるので、学校と同じ Llama 3 8B がそのまま動きます。

```bash
# Llama 3 8B — デフォルト設定でOK (QLoRA ~10GB VRAM)
python train.py --data_path data/lyrics_training_data.json

# Gemma 2 9B — 日本語が良い (~12GB VRAM)
python train.py --model_name google/gemma-2-9b-it --data_path data/lyrics_training_data.json

# Qwen 2.5 14B — 16GBギリギリ、高品質
python train.py --model_name Qwen/Qwen2.5-14B-Instruct --data_path data/lyrics_training_data.json
```

> ✅ 4060 Ti 16GB は学校の RTX 4080 (16GB) と同等のVRAMなので、
> 8B〜14B モデルは全て動作します。train.py がGPUを自動検出して設定を最適化します。

**🏫 学校 RTX 4080 × 2 (32GB VRAM):**
```bash
# Llama 3 8B — 推奨 (シングルGPU)
python train.py --data_path data/lyrics_training_data.json

# Qwen 2.5 32B — 高品質 (2枚のGPU使用)
accelerate launch train.py --model_name Qwen/Qwen2.5-32B-Instruct --data_path data/lyrics_training_data.json
```
...
# 2c. 自宅 RTX 4060 Ti 16GB で起動 (Llama 3 8B が動く！)
python serve.py \
  --host 127.0.0.1 \
  --port 8000 \
  --hf_token $HF_TOKEN &
...
学校と自宅の2台のGPU PCでUTAMEMOの歌詞生成を運用する方法。
...
1. **メイン**: 学校 4080 x2 で Llama 3 8B + vLLM → Cloudflare Tunnel で公開
2. **バックアップ**: 自宅 4060 Ti 16GB で Llama 3 8B → 別のCloudflare Tunnel
3. Render.comの`LOCAL_LLM_URL`を学校のURLに設定
4. 学校がオフラインの時だけ自宅URLに切り替え

```bash
# Render.com 環境変数
LOCAL_LLM_URL=https://学校のトンネルURL
LOCAL_LLM_API_KEY=学校のAPIキー
LYRICS_BACKEND=auto  # ダウン時はGeminiにフォールバック
```

### 方法B: 学習は学校、推論は自宅

1. **学校**: 大きいモデル (8B/32B) で **学習のみ**
2. 学習済みLoRAアダプタをGitHubにpush or USBで持ち帰る
3. **自宅**: Llama 3 8B で **推論サーバー常時稼働** (16GBなので余裕)

```bash
# 学校で学習
accelerate launch train.py --data_path data/lyrics_training_data.json --epochs 3

# LoRAアダプタをコピー (GitHub or USB)
git add output/utamemo-lyrics-lora/
git commit -m "学習済みLoRAアダプタ更新"
git push

# 自宅でpullして推論
git pull
python serve.py --hf_token $HF_TOKEN
```

### LoRAアダプタの互換性

> ⚠️ **LoRAアダプタはベースモデルに依存します。**
> 学校でLlama 3 8Bで学習したLoRAは、自宅でもLlama 3 8Bを使う必要があります。
> 
> ✅ 自宅の4060Ti (16GB) は学校の4080 (16GB) と同等のVRAMなので、
> **同じモデル・同じLoRAをそのまま使えます！**

## 📊 リソース目安

### 🏫 学校: RTX 4080 × 2 (16GB VRAM × 2)
...
| 作業 | モデル | VRAM | 時間 |
|------|--------|------|------|
| LoRA学習 Llama 3 8B (100件) | 8B | ~10GB | 1〜2時間 |
| LoRA学習 Gemma 2 9B (100件) | 9B | ~10GB | 1〜2時間 |
| LoRA学習 Qwen 2.5 14B (100件) | 14B | ~14GB | 2〜3時間 |
| 推論サーバー (Llama 3 8B) | 8B | ~10GB | 常時 |
| 推論サーバー (Gemma 2 9B) | 9B | ~10GB | 常時 |
...
- [ ] Python環境セットアップ
- [ ] CUDA / PyTorch が動作する
- [ ] Llama 3 8B で学習テスト (学校と同じモデルが使える)
- [ ] `./start_server.sh --setup` 完了 (自宅用トンネル)
- [ ] 推論サーバー起動 (Llama 3 8B)
- [ ] 学校がオフラインの時のバックアップとして動作確認
